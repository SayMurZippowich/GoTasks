...
func TestPopIndex(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			elem := DelElement(&tc.input, tc.inputIdx)
			if !reflect.DeepEqual(tc.input, tc.expected.slice) {
				t.Errorf("\ngot\t\t%v\nwant\t%v", tc.input, tc.expected.slice)
			}
...
func TestAppend(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			res := Append(tc.in.slice1, tc.in.slice2)
			if !reflect.DeepEqual(res, tc.expected) {
				t.Errorf("\ngot\t\t%v\nwant\t%v", res, tc.expected)
			}
		})
...
func TestRemove(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			res := Difference(&tc.in.slice1, &tc.in.slice2)
			if tc.expected == nil && res != nil {
				t.Fatalf("\ngot\t\t%v\nwant\tnil slice", res)
			}
			if !reflect.DeepEqual(res, tc.expected) {
				t.Errorf("\ngot\t\t%v\nwant\t%v \nCase name: %s", res, tc.expected, tc.name)
			}
		})
	}
}
...
func TestRemoveInplace(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			res := RemoveInplace(tc.in.slice1, tc.in.slice2)
			sort.Ints(res)
			if tc.expected == nil && res != nil {
				t.Fatalf("\ngot\t\t%v\nwant\tnil slice", res)
			}
...
func TestOffsetLeft(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ShiftLeftOn(&tc.in, tc.offset)
			if !reflect.DeepEqual(tc.in, tc.expected) {
				t.Errorf("\ngot\t\t%v\nwant\t%v\n Case name: %s", tc.in, tc.expected, tc.name)
			}
		})
	}
}
